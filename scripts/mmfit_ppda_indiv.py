...
from scripts.utils import run_test_analysis, wandb_logging, eval_model, eval_one_epoch, train_one_epoch
...
def get_config():
    ...
    parser.add_argument(
        ...
        type=int,
        default=3,
        help="magnitude joint combination index",
    )
    ...
    config = {
        ...
        "mag_joint_comb": args.mag_joint_comb,
        "aug_config": {
...
if __name__ == "__main__":
    ...
    config = get_config()
    config["path_processed"] = f"../data/{config['dataset_name']}"
    WANDB_PROJECT = f"ppda_{config['dataset_name']}"
    ...
    if len(aug_names) == 0:
        warnings.warn("No augmentation method is enabled. ")
        WANDB_RUN_NAME = f"mmfit_ppda_identity_{randint}"
    else:
        WANDB_RUN_NAME = f"mmfit_ppda_{aug_names[0]}_{randint}"
    wandb.init(
    ...
    # Only load the first 13 subjects (will be modified when testing different splits)
    ...
    for recording_id in recording_ids:
        ...
        sbj_pkl_file = sbj_pkl_files[0]
        print("Loading WIMUSim parameters from: ", sbj_pkl_file)
        with open(sbj_pkl_file, "rb") as f:
    ...
    if config["aug_config"].get("magscale", None) is not None:
        ...
        elif config["mag_joint_comb"] == 1:
            joint_names = ["NECK", "R_SHOULDER", "R_ELBOW", "L_SHOULDER", "L_ELBOW"]
        elif config["mag_joint_comb"] == 2:
            joint_names = ["PELVIS", "NECK", "R_SHOULDER", "L_SHOULDER"]
        elif config["mag_joint_comb"] == 3:
            joint_names = ["NECK", "R_SHOULDER", "L_SHOULDER"]
        else:
            # config["mag_joint_comb"] == 3
            joint_names = [
                "NECK",
                "R_SHOULDER",
                "L_SHOULDER",
        ...
        if config["mag_joint_comb"] == 0:
            joint_idx = [
                ...
                18,
                19,
            ]  # PELVIS, NECK, L_SHOULDER, L_ELBOW, R_SHOULDER, R_ELBOW
        elif config["mag_joint_comb"] == 1:
            joint_idx = [10, 15, 16, 18, 19]  # Without pelvis
        elif config["mag_joint_comb"] == 2:
            joint_idx = [2, 10, 15, 18]  # Without elbows
        elif config["mag_joint_comb"] == 3:
            joint_idx = [10, 15, 18]  # Without pelvis and elbows
        else:
            # By Default
            joint_idx = [
                10,
