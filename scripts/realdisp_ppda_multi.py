...
from scripts.utils import eval_model, eval_one_epoch, train_one_epoch_phase1
...
if __name__ == "__main__":
    ...
    WANDB_PROJECT = "simuaug_realdisp_n_sub_4"

    WANDB_ENTITY = os.environ.get("WANDB_ENTITY", "SPECIFY_YOUR_WANDB_ENTITY")
    randint = random.randint(0, 1000)

    WANDB_RUN_NAME = f"realdisp_ppda_{config['n_sub']}_{config['comb_idx']}_{randint}"
    wandb.init(
    ...
    # augmentor_dict = {"identity": identity}
    group1 = [identity]
    group2 = [identity]
    group3 = [identity]
    group4 = [identity]
    ...
    for i, sigma in enumerate([0.1, 0.2, 0.4, 0.6]):
        ...
        # augmentor_dict[f"magscale_{i}"] = magscale
        group1.append(magscale)

    for i, (knot, sigma) in enumerate([(2, 0.2), (2, 0.4), (4, 0.2), (4, 0.4)]):
        magwarp = MagnitudeWarping(
            sigma=sigma, knot=knot, mean_center=False, device=device
        )
        # augmentor_dict[f"magwarp_{i}"] = magwarp
    ...
    for i, (scale_min, scale_max) in enumerate(
        ...
        # augmentor_dict[f"timescale_{i}"] = timescale
    ...
    for i, (knot, max_speed_ratio) in enumerate(
        ...
        # augmentor_dict[f"timewarp_{i}"] = timewarp
    ...
    for i, (range_x, range_y, range_z) in enumerate(
        ...
        # augmentor_dict[f"rotation_{i}"] = rotation
    ...
    for i, sigma in enumerate([0.05, 0.10, 0.15, 0.20]):
        ...
        # augmentor_dict[f"noisebias_{i}"] = noise_bias
