...
from models.utils import init_weights, AverageMeter, Logger

# from models.utils import run_train_analysis, run_test_analysis
from scripts.realworld_baseline import run_test_analysis, wandb_logging

# from scripts.realdisp_stda_scaling_opt import train_one_epoch
from scripts.realdisp_baseline import eval_model, eval_one_epoch
from scripts.realdisp_stda_indiv import train_one_epoch
from simuaug.dataloaders import PPDADataLoader, RealWorldCustomSampler
...
def train_model_with_ppda(
    ...
    sampler = RealWorldCustomSampler(
        dataset=train_data_sim,
        batch_size=batch_size_train,
        max_list_ids_per_batch=8,  # 2 ** 3
    )
...
def get_config():
    ...
    parser.add_argument("--stride", type=int, default=15, help="Stride size")
    parser.add_argument(
        "--dataset_name", type=str, default="realworld", help="Name of the dataset"
    )
    parser.add_argument(
        "--train_prefix",
        type=str,
...
if __name__ == "__main__":
    ...
    if len(aug_names) == 0:
        ...
        WANDB_RUN_NAME = f"realworld_ppda_identity_{randint}"
    else:
        WANDB_RUN_NAME = f"realworld_ppda_{aug_names[0]}_{randint}"
    wandb.init(
    ...
    elif config["dataset_name"] == "realworld_n_sub3":
        subject_ids = [1, 2, 3, 9, 10, 11, 12, 13, 14, 15]
    else:
