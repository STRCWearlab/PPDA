...
from scripts.utils import train_one_epoch, eval_model, eval_one_epoch, run_test_analysis, wandb_logging
from simuaug.dataloaders import STDADataLoader

import argparse


def train_model_with_aug(
    model,
    train_data,
    val_data,
    test_data,
    ...
    loader_train_aug = STDADataLoader(
        train_data,
        batch_size=batch_size_train,
        shuffle=True,
        worker_init_fn=np.random.seed(int(seed)),
...
def get_config():
    ...
    parser.add_argument(
        "--jitter", action="store_true", help="Enable jittering augmentation"
    )
    parser.add_argument(
        "--jitter_sigma",
        type=float,
    ...
    config = {
        ...
        "aug_config": {
            ...
            "jitter": None,  # Default to None if --jitter is not enabled
...
if __name__ == "__main__":
    ...
    WANDB_RUN_NAME = f"realworld_stda_{aug_names[0]}_{randint}"
    wandb.init(
    ...
    elif config["aug_config"].get("jitter", None) is not None:
        log_sigma = torch.tensor(
            np.log(config["aug_config"]["jitter"]["sigma"]),
            requires_grad=False,
            device=device,
        )
        jitter = Jittering(log_sigma, device=device)
        sub_policies = [[identity], [jitter]]
        a_subpolicies = torch.tensor(
            [1 - aug_prob, aug_prob], device=device, requires_grad=True
        )
        aug_params = [a_subpolicies, log_sigma]
